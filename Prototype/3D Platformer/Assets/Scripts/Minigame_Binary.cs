...
public class Minigame_Binary : Minigame {
    ...
    [SerializeField] private float initialHealth = 75.0f;
    private float currentHealth = 75.0f;

    [SerializeField] private float timeToComplete = 15.0f;
    ...
    void Start () {
        for (int i = 0; i < 11; i++)
        {
            Minigame_BinarySet set = new Minigame_BinarySet(transform.Find("Main_Panel").Find("Screen").Find("BinarySets_Panel").GetChild(i).GetComponentInChildren<Text>());
            binarySets.Add(set);
        }
    ...
    private void MonitorHealth()
    {
        ...
        transform.Find("Main_Panel").Find("Screen").Find("Health_Panel").GetComponentInChildren<Text>().text = "<b>Access</b> " + ((currentHealth / initialHealth) * 100.0f).ToString("00.0") + "%";
    }

    private void MonitorTimeLeft()
    {
        ...
        transform.Find("Main_Panel").Find("Screen").Find("CommandLine_Panel").GetComponentInChildren<Text>().text = "<b><System>():</b> Security Breach Detected. Alerting Security In " + timeLeft.ToString("00.0") + " seconds.";
    }
